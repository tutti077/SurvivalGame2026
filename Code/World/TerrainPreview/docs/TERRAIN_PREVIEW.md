...
**Clover Hills (current):** Voronoi cells (~**400 m** spacing) pick candidate centers. **Hill Density** decides which cells become big hills (seeded); skipped cells form **low plateau / gap floors**. Peaks SoftCap at **`BiomeCloverMaxHeightMeters`** (default **100 m**). **Surface bumps are applied after SoftCap** (and land-display ceiling includes micro headroom) so they are not clipped off plateaus — smooth FBM only (no hash-cell steps: those aliased into vertical spikes on the 2 m vertex grid), with a slow envelope varying bump amount across the biome (0.35–1.3× amp) and a gain + soft-clip so bumps actually reach the configured ±amplitude (raw FBM averages toward flat). The main layer is domain-warped so bumps cluster instead of reading as even noise blobs. Defaults ~**±1.5 m** at ~**24 m** wavelength plus a ripple at ~0.4× wavelength and a dense ~9 m detail layer (old JSON 0 → those defaults; wavelengths floored at 8 m). Tune via **Biome Terrain - Clover Micro Wavelength (m)** / **Clover Micro Amplitude (m)**. Console logs one `[CloverMicro] probe …` line on first Clover sample so you can confirm bumps ≠ 0.
...
**Mesh LOD:** distance-based both ways with hysteresis — chunks outside the high-priority radius build at **Far Stream Vertices Per Side** (17 ≈ 4 m), promote in place to full detail inside **High-Priority Radius**, and demote back only beyond radius + **LOD Demote Margin (chunks)** so boundary chunks never flip-flop. LOD distance is **3D** (altitude vs each chunk's cached ground height counts; climbing demotes the ground below, diving promotes it; refreshes also trigger on ~48 m altitude change), while **colliders and entity population stay on horizontal range** — physics under a falling player never switches off with height. The initial load uses the same distance rule (no all-full-detail start). Every chunk, any LOD, snaps its **border verts onto the far-LOD lattice**, so full↔far seams are watertight by construction — no neighbor tracking, no tears in either direction. Far Stream Vertices Per Side must keep (full−1) divisible by (far−1) (e.g. 33/17) or border snapping turns off.
