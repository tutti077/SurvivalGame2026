...
public sealed class InventorySlotPanel : Panel
{
	...
	public override void OnButtonEvent( ButtonEvent e )
	{
		...
		// Right-click is handled by exactly one poll path (overlay on press while the menu is
		// open, hotbar pointer poll on release while closed) — handling it here too deposited
		// twice per click. Just swallow the event so nothing else reacts.
		if ( IsSecondaryMouseButton( e.Button ) )
		{
